 ...
 */
public class MusikplayerDialog extends MyStage {

	private static final String MEDIA_URL = new File(
			"D:/Internet/Eigene_Dateien/Eigene Musik/Gesamt/Bryan Adams/Bryan Adams - Summer of '69.mp3")
			.toURI().toString();

	private MediaPlayer mediaPlayer;

	public MusikplayerDialog(Stage primaryStage) {
		init(primaryStage);
		play();
	}
	...
	static class PlayerPane extends BorderPane {
		...
		public PlayerPane(final MediaPlayer mp) {
			...
			mediaTopBar = HBoxBuilder.create()
					.padding(new Insets(5, 10, 5, 10)).alignment(Pos.CENTER)
					.opacity(0).build();
